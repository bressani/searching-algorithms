...
class BinarySearchKtTest {
    ...
    @ParameterizedTest
    @MethodSource("provide test arguments for binary search")
    fun `test binary search`(expected: Int, array: IntArray, searchNumber: Int) {
        val actual = binarySearch(searchNumber, array)

        assertEquals(expected, actual)
    }

    fun `provide test arguments for binary search`(): Stream<Arguments> =
            Stream.of(
